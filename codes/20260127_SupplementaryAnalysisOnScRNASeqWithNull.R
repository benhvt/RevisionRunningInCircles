...
results_application <- read.csv("results/SupplementaryResultsApplicationOnScRNASeqWithNull.csv") %>%
  ...
  mutate(Hyp = rep(c(rep("H1", length(H1_genes)),
                     rep("H0", length(H0_genes))),4))
...
plt_scatter_pval_H0 / plt_scatter_pval_H1 + 
...
ggsave(filename = "Supplementary Figures/SuppFigure_ApplicationTwoCellPopsWithNull.pdf",
       width = 150, 
       height = 250, 
       units = "mm")
...
results_application %>%
  ggplot() +
  aes(Hyp, y= ARI, colour = Hyp) +
  geom_boxplot() +
  facet_grid(ClusteringName~ThinningName)  

results_application %>% group_by(Thinning,Clustering, Hyp) %>% summarise(mean(pvalue<0.05)) %>% filter(Hyp == 'H0')
results_application %>% group_by(Thinning,Clustering, Hyp) %>% summarise(mean(pvalue<0.05)) %>% filter(Hyp == 'H1')

# results_application %>% filter(Clustering == "Univariate", Thinning == "Conditional", Hyp == "H0", pvalue<0.05)
